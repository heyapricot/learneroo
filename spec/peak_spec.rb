...
context "finding the peaks" do
  ...
  it "tells if a node is peak" do
    my_map.fromArray(tests[2])
    expect(my_map.is_peak(my_map.nodes[0])).to eql false

  end


  xit "returns an array of values higher than their connections" do
    tests = Array.new
    peaks = Array.new
...
context "finding the peaks of highest area" do
  xit "returns the two peaks with highest area" do
    graph = Graph.new
